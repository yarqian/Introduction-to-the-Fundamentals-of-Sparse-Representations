...
for i=1:size(A,2)
    ...
    while empty_atom_flag
        ...
        rect_w = randi([5,20]);
        rect_h = randi([5,20]);
        rect_value = 2*randi(2)-3;
        %
        rect = rect_value*ones(rect_h, rect_w);
        % position
        rect_x = randi([1,n-size(rect,1)]);
        rect_y = randi([1,n-size(rect,2)]);
        atom(rect_x:rect_x+rect_h-1, rect_y:rect_y+rect_w-1) = rect;
...
for experiment = 1:num_experiments
    ...
    suport_x = find(x0);   
    x_oracle = oracle(A_eff, b, suport_x);
...
for experiment = 1:num_experiments
    ...
    for p_ind = 1:num_values_of_p
        ...
        cur_mse = norm(b_omp-b0)^2/40.0^2;
